"""ROS2 node that orchestrates Franka pick-and-place via MoveGroup action + py_trees.

Uses the MoveGroup action server (from the already-running move_group node)
for trajectory planning and execution. Gripper is controlled directly via
/joint_command topic. py_trees BehaviorTree sequences the task phases.
"""
...
from panda_pick_place.bt_nodes import build_pick_place_tree
...
class PickPlaceContext:
    ...
    def _send_move_group_goal(self, goal_msg: MoveGroup.Goal) -> bool:
        """Send a goal to move_group and block until result."""
    ...
    def move_to_pose(self, position: list[float], orientation: list[float]) -> bool:
        """Plan and execute via the MoveGroup action server."""
        ...
        self._logger.info(f"Sending MoveGroup goal: pos={position}")
    ...
    def set_gripper(self, width: float) -> None:
        """Publish gripper commands both via ros2_control and direct topic."""
        msg = Float64MultiArray()
        msg.data = [width]
        self._gripper_pub.publish(msg)

        cmd = JointState()
        cmd.name = ["panda_finger_joint1", "panda_finger_joint2"]
        cmd.position = [width, width]
...
def main():
    ...
    node = rclpy.create_node("pick_place_orchestrator")
    ...
    logger.info("Waiting for cube and target poses from Isaac Sim ...")
